...
export default function AdminDashboardView({ onNavigateHome }) {
  ...
  const [clickToCallCount, setClickToCallCount] = useState(() => {
    return parseInt(localStorage.getItem('havenridge_metric_calls') || '0', 10);
  });
  const [clickToEmailCount, setClickToEmailCount] = useState(() => {
    return parseInt(localStorage.getItem('havenridge_metric_emails') || '0', 10);
  });
  ...
  useEffect(() => {
    async function loadSupabaseData() {
      try {
        ...
        // Fetch Metrics
        const { data: metricsData } = await supabase.from('site_metrics').select('*');
        if (metricsData) {
          metricsData.forEach(m => {
            if (m.id === 'calls') setClickToCallCount(m.count);
            if (m.id === 'emails') setClickToEmailCount(m.count);
          });
        }
